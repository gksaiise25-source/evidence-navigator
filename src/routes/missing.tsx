      ...
      <Panel>
        <table className="w-full text-left text-sm">
          <thead className="mono-xs text-muted-foreground">
            <tr><th className="py-1 pr-3">CATEGORY</th><th className="py-1 pr-3">RECORDS</th><th className="py-1 pr-3">STATUS</th><th className="py-1 pr-3">NOTE</th></tr>
          </thead>
          <tbody>
            {bundle.coverage.map((c) => (
              <tr key={c.type} className="border-t border-border/50">
                ...
                <td className="py-1.5 pr-3"><Tag tone={c.status === "AVAILABLE" ? "green" : "warn"}>{c.status}</Tag></td>
                <td className="mono-xs py-1.5 pr-3 text-muted-foreground">{c.note}</td>
              </tr>
            ))}
          </tbody>
        </table>
